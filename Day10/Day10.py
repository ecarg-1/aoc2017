...
#pt 1
a = run(1, pt1_in, [i for i in range(elements)]) 
print(a[0]*a[1])
#pt 2
rounds, suffix = 64, [17, 31, 73, 47, 23]
pt2_in = ','.join([str(i) for i in pt1_in])
pt2_in = [ord(pt2_in[i]) for i in range(len(pt2_in))] + suffix
b = run(rounds, pt2_in, [i for i in range(elements)])
print(xor(b))